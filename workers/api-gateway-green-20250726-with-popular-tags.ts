...
// Popular tags KV data structure
...
function generatePopularTagsETag(data: any): string {
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/api/popular-tags') {
      ...
      // Generate ETag
      const etag = generatePopularTagsETag(responseData)
      ...
      if (ifNoneMatch === etag) {
        const notModifiedResponse = new Response(null, {
          ...
          headers: {
            ...
            'X-Worker-Version': 'green-20250726-with-popular-tags'
          }
        })
        const origin = request.headers.get('Origin')
      ...
      // Return JSON response with popular tags
      const popularTagsResponse = new Response(JSON.stringify(responseData), {
        status: 200,
        headers: {
          'content-type': 'application/json',
          'cache-control': 'public, max-age=1800, s-maxage=3600, stale-while-revalidate=86400',
      ...
      const origin = request.headers.get('Origin')
      return applyCORSHeaders(popularTagsResponse, origin, securityHeaders)
    }
