 ...
 */
export function decodeRankingItem(item) {
    if (!item)
        return item;
    return {
        ...item,
        ...
        tags: item.tags ? item.tags.map((tag) => decodeHtmlEntities(tag)) : item.tags
    };
}
/**
 ...
 */
export function decodeRankingData(data) {
    if (!data)
        return data;
    ...
    return {
        ...
        popularTags: data.popularTags ? data.popularTags.map((tag) => {
