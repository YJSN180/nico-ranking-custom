...
interface MylistOperationsContextType {
  mylists: Mylist[]
  isLoading: boolean
  addVideoToMylist: (mylistId: string, video: Partial<MylistVideo>) => Promise<boolean>
  removeVideoFromMylist: (mylistId: string, videoId: string) => Promise<boolean>
  isVideoInAnyMylist: (videoId: string) => Promise<{ inMylist: boolean; mylistIds: string[] }>
  createMylist: (name: string, description?: string) => Promise<string | null>
}

const MylistOperationsContext = createContext<MylistOperationsContextType | null>(null)

interface MylistOperationsProviderProps {
  children: ReactNode
}

export function MylistOperationsProvider({ children }: MylistOperationsProviderProps) {
  ...
  const contextValue: MylistOperationsContextType = {
    mylists,
    isLoading,
    addVideoToMylist,
  ...
  return (
    <MylistOperationsContext.Provider value={contextValue}>
      {children}
    </MylistOperationsContext.Provider>
  )
}

export function useMylistOperations(): MylistOperationsContextType {
  const context = useContext(MylistOperationsContext)
  if (!context) {
    throw new Error('useMylistOperations must be used within a MylistOperationsProvider')
  }
  return context
}