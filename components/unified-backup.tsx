...
export function UnifiedBackup() {
  ...
  const [pendingImportData, setPendingImportData] =
    useState<UnifiedBackupData | null>(null)
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      if (pendingImportData.data.ngList) {
        try {
          const conflicts = detectExtendedConflicts(
            ngList,
            pendingImportData.data.ngList.ngList,
          )
          const result = await importExtendedNGListData(
            pendingImportData.data.ngList,
            'merge',
          )
          results.push(`✅ NGリスト: ${result.imported.totalItems}件インポート`)
        } catch (error) {
          errors.push(
            `❌ NGリスト: ${error instanceof Error ? error.message : 'エラー'}`,
          )
        }
      }
      ...
      if (pendingImportData.data.genreOrder) {
        try {
          localStorage.setItem(
            ...
            JSON.stringify(pendingImportData.data.genreOrder),
          )
          results.push(
            `✅ ジャンル並び替え: ${pendingImportData.data.genreOrder.length}件設定`,
          )
        } catch (error) {
          errors.push(
            `❌ ジャンル並び替え: ${error instanceof Error ? error.message : 'エラー'}`,
          )
        }
      }
      ...
      if (pendingImportData.data.customRankings) {
        try {
          ...
          for (const ranking of pendingImportData.data.customRankings) {
            const existing = customRankings.find(
              (r) => r.title === ranking.title,
            )
            if (existing) {
          ...
          errors.push(
            `❌ カスタムランキング: ${error instanceof Error ? error.message : 'エラー'}`,
          )
        }
      }
      ...
      if (pendingImportData.data.mylists) {
        try {
          const conflicts = await detectMylistConflicts(
            pendingImportData.data.mylists,
          )
          const result = await importMylistData(
            pendingImportData.data.mylists,
            'safe_add',
          )
          results.push(
            `✅ マイリスト: ${result.created.mylists}件, 動画${result.created.videos}件インポート`,
          )
        } catch (error) {
          errors.push(
            `❌ マイリスト: ${error instanceof Error ? error.message : 'エラー'}`,
          )
        }
      }
    ...
    <div className={styles.genreOrderBackup}>
      <div className={styles.backupActions}>
        ...
        <label
          ...
          data-testid="import-unified-button"
        >
          <input
            type="file"
            accept=".json"
            ...
            <div className={styles.exportStats}>
              ...
              <div className={styles.statItem}>
                ...
                <span className={styles.statValue}>
                  {genreOrderItems.length}件
                </span>
              </div>
              <div className={styles.statItem}>
                <span className={styles.statLabel}>カスタムランキング:</span>
                <span className={styles.statValue}>
                  {customRankings.length}件
                </span>
              </div>
              <div className={styles.statItem}>
      ...
      {importConfirmOpen && pendingImportData && (
        ...
        >
          ...
          >
            ...
            <p>選択されたバックアップファイルから以下のデータを復元します。</p>
