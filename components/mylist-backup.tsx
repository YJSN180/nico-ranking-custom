...
export function MylistBackup() {
  ...
  const [importResult, setImportResult] = useState<MylistImportResult | null>(
    null,
  )
  ...
  const handleConflictResolution = async (
    resolution: 'safe_add' | 'smart_merge' | 'complete_overwrite',
  ) => {
            ...
            <div className={styles.conflictSummary}>
              <div className={styles.importStats}>
                <div className={styles.statItem}>
                  ...
                  <span className={styles.statValue}>
                    {conflictData.conflicts.summary.importingMylists}マイリスト,{' '}
                    {conflictData.conflicts.summary.importingVideos}動画
                  </span>
                </div>
                <div className={styles.statItem}>
                  <span className={styles.statLabel}>競合検出:</span>
                  <span className={styles.statValue}>
                    {conflictData.conflicts.summary.totalConflictingMylists}
                    マイリスト,{' '}
                    {conflictData.conflicts.summary.totalConflictingVideos}動画
                  </span>
                </div>
              </div>
              ...
              {conflictData.conflicts.conflicts.mylistIds.length > 0 && (
                <div className={styles.conflictDetail}>
                  <div className={styles.conflictType}>
                    🔄 ID重複マイリスト (
                    {conflictData.conflicts.conflicts.mylistIds.length}件)
                  </div>
                  <div className={styles.conflictList}>
                    {conflictData.conflicts.conflicts.mylistIds
                      .slice(0, 3)
                      .map((conflict, index) => (
                        <div key={index} className={styles.conflictItem}>
                          ...
                          」
                          <small>
                            ({conflict.existingVideoCount}→
                            {conflict.importingVideoCount}動画)
                          </small>
                        </div>
                      ))}
              ...
              {conflictData.conflicts.conflicts.mylistNames.length > 0 && (
                <div className={styles.conflictDetail}>
                  <div className={styles.conflictType}>
                    📝 名前重複マイリスト (
                    {conflictData.conflicts.conflicts.mylistNames.length}件)
                  </div>
                  <div className={styles.conflictList}>
                    {conflictData.conflicts.conflicts.mylistNames
                      .slice(0, 3)
                      .map((conflict, index) => (
                        <div key={index} className={styles.conflictItem}>
                          「{conflict.name}」
                          <small>
                            ({conflict.existingVideoCount}動画 +{' '}
                            {conflict.importingVideoCount}動画)
                          </small>
                        </div>
                      ))}
                    {conflictData.conflicts.conflicts.mylistNames.length >
                      3 && (
                      <div className={styles.moreConflicts}>
                        ...他
                        {conflictData.conflicts.conflicts.mylistNames.length -
              ...
              {conflictData.conflicts.conflicts.videos.length > 0 && (
                <div className={styles.conflictDetail}>
                  <div className={styles.conflictType}>
                    🎬 動画重複 (
                    {conflictData.conflicts.conflicts.videos.length}件)
                  </div>
                  <div className={styles.conflictList}>
                    {conflictData.conflicts.conflicts.videos
                      .slice(0, 2)
                      .map((conflict, index) => (
                        <div key={index} className={styles.conflictItem}>
                          「{conflict.title}」
                    ...
                    {conflictData.conflicts.conflicts.videos.length > 2 && (
                      <div className={styles.moreConflicts}>
                        ...他
                        {conflictData.conflicts.conflicts.videos.length - 2}件
                      </div>
                    )}
                  </div>
                </div>
              ...
              <button
                onClick={() => {
                  const selected = document.querySelector(
                    'input[name="mylist-resolution"]:checked',
                  ) as HTMLInputElement
                  if (selected) {
          ...
          {importResult.success ? (
            <div>
              ...
              <div className={styles.resultDetails}>
                <div>
                  追加されたマイリスト: {importResult.created.mylists}件
                </div>
                <div>追加された動画: {importResult.created.videos}件</div>
                {importResult.overwritten.mylists > 0 && (
                  <div>
                    上書きされたマイリスト: {importResult.overwritten.mylists}件
                  </div>
                )}
                {importResult.overwritten.videos > 0 && (
                  <div>
                    上書きされた動画: {importResult.overwritten.videos}件
                  </div>
                )}
                {importResult.renamed.mylists.length > 0 && (
                  <div className={styles.renamedInfo}>
                    リネームされたマイリスト:{' '}
                    {importResult.renamed.mylists.length}件
                    <div className={styles.renamedList}>
                      {importResult.renamed.mylists
                        .slice(0, 3)
                        .map((renamed, index) => (
                          <div key={index}>
                            「{renamed.original}」→「{renamed.renamed}」
                          </div>
                        ))}
                      {importResult.renamed.mylists.length > 3 && (
                        <div>
                          ...他{importResult.renamed.mylists.length - 3}件
                        </div>
                      )}
                    </div>
                  </div>
                )}
                {importResult.skipped.mylists + importResult.skipped.videos >
                  0 && (
