...
describe('Edge Video Stats API', () => {
  ...
  describe('GET /api/edge/video-stats', () => {
    ...
    it('should set proper cache headers', async () => {
      ...
      // Check if getCacheHeaders is used properly
    ...
    it('should handle partial data from KV gracefully', async () => {
      // Set environment variables
      process.env.CLOUDFLARE_ACCOUNT_ID = 'test-account'
      process.env.CLOUDFLARE_KV_NAMESPACE_ID = 'test-namespace'
      process.env.CLOUDFLARE_API_TOKEN = 'test-token'
      ...
      expect(response.status).toBe(200)
      expect(data.stats).toEqual({
        'sm123': kvData.stats['sm123']
        // sm456 is not included since it's not in KV
      })
      expect(data.count).toBe(1) // Only 1 video found
    })
    ...
    it('should handle KV errors gracefully', async () => {
      ...
      // Mock KV fetch to return error
      ;(global.fetch as any).mockResolvedValueOnce({
        ok: false,
        status: 404,
        statusText: 'Not Found',
        text: async () => 'KV namespace not found'
      })
      ...
      expect(response.status).toBe(200)
      expect(data.stats).toEqual({}) // Empty stats returned
      expect(data.count).toBe(0) // No videos found
    })
