// カスタムフィルタリング機能のテスト（JavaScript版）

// フィルタリングロジックの実装を再現（テスト用）
function hasMatchingTag(item, condition) {
...
function applyCustomFilters(items, conditions) {
...
function runCustomFilterTests() {
  ...
  for (const testCase of testCases) {
    ...
    console.log(`条件:`, JSON.stringify(testCase.conditions, null, 2))
    ...
    if (!isPass) {
      ...
      // デバッグ情報：各動画がなぜ含まれた/含まれなかったかを分析
      console.log(`  詳細分析:`)
      for (const video of mockData) {
        const shouldInclude = expectedIds.includes(video.id)
        const actuallyIncluded = resultIds.includes(video.id)
        if (shouldInclude !== actuallyIncluded) {
          console.log(`    ${video.id} (${video.title}):`)
          console.log(`      期待: ${shouldInclude ? '含む' : '含まない'}, 実際: ${actuallyIncluded ? '含む' : '含まない'}`)
          console.log(`      タグ: ${JSON.stringify(video.tags)}`)
          console.log(`      タグ詳細: ${JSON.stringify(video.tagDetails)}`)
          
          // 各条件との照合結果
          for (const condition of testCase.conditions) {
            const matches = hasMatchingTag(video, condition)
            console.log(`      条件 ${condition.operator} ${condition.tag} (${condition.tagType}): ${matches}`)
          }
        }
      }
    }
